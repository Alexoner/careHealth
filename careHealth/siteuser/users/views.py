...
class SiteUserRegisterView(user_defined_mixin(), SiteUserMixIn, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user = InnerUser.objects.create(
            email=email, passwd=passwd, username=username)
        request.session['uid'] = user.user.id
...
class SiteUserResetPwStepOneView(user_defined_mixin(), SiteUserMixIn, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        body = loader.render_to_string(
            self.reset_passwd_email_template, context)
        # 异步发送邮件
        body = unicode(body)
...
class SiteUserResetPwStepTwoView(user_defined_mixin(), SiteUserMixIn, View):
    ...
    def get(self, request, *args, **kwargs):
        token = kwargs['token']
        try:
            self.uid = signing.loads(
                token, key=self.sign_key, max_age=self.reset_passwd_link_expired_in * 3600)
...
class SiteUserChangePwView(user_defined_mixin(), SiteUserMixIn, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        InnerUser.objects.filter(
            user_id=request.siteuser.id).update(passwd=password)
        # 清除登录状态
        try:
...
def social_login_callback(request, sitename):
    ...
    try:
        user = SocialUser.objects.get(
            site_uid=site.uid, site_name=site.site_name)
        SiteUser.objects.filter(id=user.user.id).update(
            username=site.name, avatar_url=site.avatar)
